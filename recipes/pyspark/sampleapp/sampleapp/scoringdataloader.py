...
def load(configProperties, spark):
    ...
    scoringDataLocation = str(configProperties.get("scoringDataLocation"))
    scoringSchema =  StructType([
        StructField("id", LongType()),
        StructField("user", StringType()),
        StructField("text", StringType())
    ])
    testData = spark.read.schema(scoringSchema).format("csv").option("mode", "DROPMALFORMED").csv(scoringDataLocation)
    return testData
