...
def load(configProperties, spark):
    ...
    print("Training: In UserDataLoader load")
    ...
    print("Training: After loading dataset")
    pd.show()

    print("Training: Total number of rows: " + str(pd.count()))

    # Convert isHoliday boolean value to Int
    pd = pd.withColumn("isHoliday", col("isHoliday").cast(IntegerType()))

    print("Training: After converting isHoliday")
    pd.show()
    ...
    print("Training: After getting date and year")
    pd.show()

    # Convert the date to TimestampType
    pd = pd.withColumn("tx_date", to_date(unix_timestamp(pd["date"], "MM/dd/yy").cast("timestamp")))
    print("Training: After converting the date column")
    pd.show()
    ...
    print("Training: After calculating the weeklySales Values")
    pd.show()
    ...
    print("Training: Total number of rows: " + str(train.count()))
    train.show()
